...
Sensitive identifiers (SSN, full credit/debit card numbers, CVV, full account numbers) must never be repeated back to the representative in chat — speak only the last 4 digits of card or account numbers, and do not voice SSNs or CVVs at all. If the rep needs to verify the caller's identity against a sensitive field, route them to the dedicated out-of-band identity-verification flow rather than reading the value over chat.
